...
@Testcontainers
@Transactional
@DisplayName("充电站系统集成测试")
class ChargingStationIntegrationTest {

    @Container
    static MySQLContainer<?> mysql = new MySQLContainer<>("mysql:8.0")
            .withDatabaseName("chargingstation_test")
            .withUsername("test")
            .withPassword("test");
    ...
    void setUp() {
        ...
        locationRequest.setDescription("测试用充电站");

        evseRequest = new CreateEvseRequest("CN*TST*EVSE001");
        evseRequest.setDescription("测试EVSE设备1号");
        ...
        connectorRequest.setDescription("CCS2直流快充接口");
    }
    ...
    void testCompleteChargingStationWorkflow() throws Exception {
        ...
        MvcResult locationResult = mockMvc.perform(post("/api/v1/locations")
                ...
                .andExpect(jsonPath("$.name").value("测试充电站"))
                .andExpect(jsonPath("$.address").value("北京市朝阳区测试路123号"))
        ...
        MvcResult evseResult = mockMvc.perform(post("/api/v1/locations/{locationId}/evses", locationId)
                ...
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.evseId").value("CN*TST*EVSE001"))
        ...
        mockMvc.perform(get("/api/v1/locations/{locationId}", locationId))
                ...
                .andExpect(jsonPath("$.evses[0].evseId").value("CN*TST*EVSE001"))
        ...
        // 7. 通过EVSE标识符查询EVSE
        mockMvc.perform(get("/api/v1/evses/evse-id/{evseId}", "CN*TST*EVSE001"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.evseId").value("CN*TST*EVSE001"))
                .andExpect(jsonPath("$.status").value("AVAILABLE"));
    }
    ...
    void testGeographicBoundsQuery() throws Exception {
        ...
        CreateLocationRequest location1 = new CreateLocationRequest(
                "充电站1", "地址1", BigDecimal.valueOf(39.9042), BigDecimal.valueOf(116.4074)
        );
        CreateLocationRequest location2 = new CreateLocationRequest(
                "充电站2", "地址2", BigDecimal.valueOf(39.9142), BigDecimal.valueOf(116.4174)
        );
        CreateLocationRequest location3 = new CreateLocationRequest(
                "充电站3", "地址3", BigDecimal.valueOf(40.0042), BigDecimal.valueOf(116.5074)
        );
        ...
        // 地理边界查询 - 应该返回前两个充电站
        mockMvc.perform(get("/api/v1/locations/bounds")
